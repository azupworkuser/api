		...
		{   
			...
			if(media_storage=="s3")
			 {  
                 ...
                 $tempName=time().rand();
			     file_put_contents("tmp/".$tempName.".mp4", $video);
			     
			     $videoFilePath = "tmp/".$tempName.".mp4";
			     
			     /********************optimize video size and upload on aws s3**************************/
			     
			     $resultFile = "tmp/".$fileName.".mp4";
			     
			     $cmd_new = "$ffmpeg_string -i $videoFilePath -c:v libx264 -crf 28 $resultFile";
                 exec($cmd_new);
                 
                 //upload files on aws s3
                 $video_url=uploadS3($resultFile,"videos");
                ...
                /********************convert video to audio**************************/
                 ...
                 if($sound_id=="null")
                 {
        			...
        			{   
                        ...
                        $resultFileMP3 = "tmp/".$fileName.".mp3";
                        $video->save($output_format, $resultFileMP3);
                        
                        $resultFileAAC = "tmp/".$fileName.".aac";
                        $command_new = "$ffmpeg_string -i $resultFileMP3 -c:a aac -b:a 192k $resultFileAAC";
                        exec($command_new);

        			    $insert_id=mysqli_insert_id($conn);
			            @copy($resultFileAAC,'upload/audio/'.$insert_id.'.aac');
        			}
                ...
                /********************end convert video to audio**************************/
                 ...
                 /********************generate jpg from video**************************/
                     ...
                     $resultFile = "tmp/".$fileName.".jpg";
                     $video
                        ->frame(FFMpeg\Coordinate\TimeCode::fromSeconds(2))
                        ->save($resultFile);
                    //upload jpg on s3
                     $thum_url=uploadS3($resultFile,"thum");
                ...
                ->save($resultFile);
                 ...
                 //upload gif on s3
                 $gif_url=uploadS3($resultFile,"gif");
                 
                /********************generate gif from the video file and upload on aws s3**************************/ 
                
                //remove all files from temp
                @unlink("tmp/".$tempName.".mp4");
                @unlink("tmp/".$fileName.".jpg");
                @unlink("tmp/".$fileName.".mp4");
                @unlink("tmp/".$fileName.".gif");
                @unlink("tmp/".$fileName.".aac");
                @unlink("tmp/".$fileName.".mp3");
			 ...
			 {  
			     ...
			     /********************optimize video size and upload on aws s3**************************/
                    ...
                    $resultFile = "upload/video/".$fileName.".mp4";
                    $cmd_new = "$ffmpeg_string -i $videoFilePath -c:v libx264 -crf 28 $resultFile";
                    exec($cmd_new);
                /********************optimize video size and upload on aws s3**************************/
                ...
                if($sound_id=="null")
                {
    			 ...
    			 /********************generate jpg from video**************************/
                    ...
                    $resultFile = "upload/thum/".$fileName.".jpg";
                    $video
                    ->frame(FFMpeg\Coordinate\TimeCode::fromSeconds(2))
                    ->save($resultFile);
                ...
                /********************generate gif from the video file and upload on aws s3**************************/
                    ...
                    $resultFile = "upload/gif/".$fileName.".gif";
                    ...
                    ->save($resultFile);
                     ...
                     //upload gif on local folder
                     //@copy($videoFilePath,$resultFile.'.gif');
...
// 	    $header_deviceid = $headers['Deviceid'];
	...
	function showAllVideos()
	{
		...
		if(isset($event_json['fb_id']))
		{
			...
			{
			    $query=mysqli_query($conn,"select * from videos order by rand() limit 100");
			}
